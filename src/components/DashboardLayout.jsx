...
import {
    ChevronDown, FileText, Database, Users, PanelLeft, Share2,
    BookOpen, LogOut, ArrowLeft, ChevronRight, MessageSquare,
    Activity, Plus, AlignLeft, Sun, Square, Check
} from 'lucide-react';
...
const DashboardLayout = () => {
    ...
    const isIngestion = location.pathname.includes('prepaid-data-ingestion') || isProcessDetailPage;

    const handleLogout = () => { setIsAirbnbOpen(false); navigate('/'); };

    const SidebarItem = ({ to, icon, label, isActive }) => (
        <NavLink
            to={to}
            className={`flex h-[34px] w-full items-center gap-2.5 overflow-hidden rounded-md px-2.5 transition-colors ${isActive ? 'bg-[#efefef] text-[#171717] font-[550]' : 'text-[#383838] hover:bg-[#00000005]'}`}
        >
    ...
    const getHeaderTitle = () => {
        if (location.pathname.includes('/knowledge-base')) return null;
        if (isProcessDetailPage) return null;
        if (location.pathname.includes('prepaid-data-ingestion')) return 'Prepaid - Data Ingestion & Invoice Extraction';
        if (location.pathname.includes('prepaid-expense-booking')) return 'Prepaid - Expense Booking';
        return 'DXC Account Reconciliation';
    };

    const getBackSlug = () => {
        if (location.pathname.includes('expense')) return 'prepaid-expense-booking';
        return 'prepaid-data-ingestion';
    };

    return (
        <div className="flex h-screen bg-[#FAFAFA] font-sans antialiased text-[#171717]">
            {!isSidebarOpen && (
                <button onClick={() => setIsSidebarOpen(true)} className="absolute top-0 left-0 z-30 flex h-10 w-10 items-center justify-center bg-transparent transition-opacity hover:opacity-100 opacity-50">
            ...
            <aside className={`fixed top-0 left-0 z-20 flex h-screen w-60 flex-col overflow-hidden bg-[#FAFAFA] transition-transform duration-150 ease-[0.4,0,0.2,1] ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="flex-1 overflow-y-auto px-2 pb-4 space-y-1 mt-2">
                    ...
                    <div className="pt-4">
                        ...
                        <SidebarItem
                            to="/done/prepaid-data-ingestion"
                            icon={<Activity />}
                            label="Prepaid - Data Ingestion"
                            isActive={location.pathname.includes('prepaid-data-ingestion') || (isProcessDetailPage && !location.state?.fromExpense)}
                        />
                        <SidebarItem
                            to="/done/prepaid-expense-booking"
                            icon={<Activity />}
                            label="Prepaid - Expense Booking"
                            isActive={location.pathname.includes('prepaid-expense-booking')}
                        />
                    </div>
                    <div className="pt-4">
                        <div className="flex items-center justify-between px-3 mb-2">
                ...
                <div className="mt-auto border-t border-[#f0f0f0] relative p-1 bg-[#FAFAFA]">
                    <button onClick={() => setIsAirbnbOpen(!isAirbnbOpen)} className="w-full flex items-center justify-between px-2.5 py-2 text-[13px] text-[#383838] hover:bg-[#00000008] rounded-md transition-colors">
                        <div className="flex items-center gap-2.5 font-[500]">
                            <div className="w-6 h-6 bg-[#ebebeb] rounded flex items-center justify-center text-black font-bold text-[11px]">D</div>
                    ...
                    {isAirbnbOpen && (
                        <div className="absolute bottom-full left-1 right-1 mb-1 bg-white border border-[#f0f0f0] rounded-lg shadow-[0_-4px_20px_rgba(0,0,0,0.05)] py-1 z-50">
            ...
            <div className={`flex-1 flex flex-col transition-all duration-150 ease-[0.4,0,0.2,1] ${isSidebarOpen ? 'ml-60' : 'ml-0'}`}>
                <header className="h-10 flex items-center justify-between px-4 bg-[#FAFAFA] relative border-b border-[#f0f0f0]">
                    <div className={`flex items-center gap-3 transition-opacity duration-150 ${!isSidebarOpen ? 'pl-8' : ''}`}>
                        <div className="flex items-center gap-2 text-[13px]">
                            {location.pathname.includes('/knowledge-base') ? (
                                <>
                                    <button onClick={() => navigate('/done/prepaid-data-ingestion')} className="hover:bg-white rounded p-1 transition-colors"><ArrowLeft className="w-3.5 h-3.5 text-[#171717]" /></button>
                                    <span className="text-[#8f8f8f] font-normal">DXC Account Reconciliation /</span>
                                    <span className="text-[#171717] font-bold">Knowledge Base</span>
                                </>
                            ) : isProcessDetailPage ? (
                                <>
                                    <button onClick={() => navigate(-1)} className="hover:bg-white rounded p-1 transition-colors"><ArrowLeft className="w-3.5 h-3.5 text-[#171717]" /></button>
                                    <span className="text-[#171717] font-[550]">DXC Account Reconciliation</span>
                                    <ChevronRight className="w-3.5 h-3.5 text-[#c9c9c9]" />
                                    <span className="text-[#171717] font-[550]">Activity Logs</span>
                                </>
                            ) : (
                                <span className="text-[#171717] font-[550]">{getHeaderTitle()}</span>
                            )}
                        </div>
                    </div>
                    <div className="absolute left-1/2 transform -translate-x-1/2">
                        <button onClick={() => setIsFeedbackModalOpen(true)} className="bg-[#efefef] text-[#8f8f8f] border border-[#f0f0f0] flex h-7.5 w-[220px] items-center justify-between px-3 py-1 rounded-md shadow-sm hover:border-[#e5e5e5] transition-all">
                    ...
                    <div className="flex items-center gap-2 px-4">
                        <button onClick={() => navigate('/done/knowledge-base?category=dir')} className="group relative p-1.5 hover:bg-white rounded-md border border-[#f0f0f0] bg-[#FAFAFA] shadow-sm transition-colors">
                            <BookOpen className="w-4 h-4 text-[#666666]" />
                            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-3 py-1.5 bg-[#171717] text-white text-[11px] font-medium rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">Knowledge Base</div>
                        </button>
                        <button className="group relative p-1.5 hover:bg-white rounded-md border border-[#f0f0f0] bg-[#FAFAFA] shadow-sm transition-colors">
                            <MessageSquare className="w-4 h-4 text-[#666666]" />
                            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 px-3 py-1.5 bg-[#171717] text-white text-[11px] font-medium rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">All chats</div>
                        </button>
                        <div className="flex items-center bg-[#f5f5f5] rounded-xl p-0.5 border border-[#ebebeb] shadow-sm ml-2" ref={queueTriggerRef}>
                            <button onClick={() => setIsQueueOpen(!isQueueOpen)} className={`flex items-center gap-1.5 px-2 py-1 rounded-lg text-[11px] font-medium transition-all ${isQueueOpen ? 'bg-white shadow-sm text-[#171717]' : 'text-[#666] hover:bg-[#eaeaea]'}`}>
                                <div className="w-3.5 h-3.5 text-blue-500"><Square size={14} strokeWidth={2.5} /></div>
                                <span className="text-[#8f8f8f] px-1 text-[9px] font-semibold">{counts.open}</span>
                            </button>
                            <div className="w-px h-3 bg-[#e0e0e0] mx-0.5" />
                            <button onClick={() => setIsQueueOpen(!isQueueOpen)} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[11px] font-medium text-[#666] hover:bg-[#eaeaea] transition-all">
                                <div className="w-3.5 h-3.5 text-[#666]"><AlignLeft size={14} strokeWidth={2} /></div>
                            ...
                            <div className="absolute top-full right-0 mt-2 z-50">
                                <FeedbackQueuePanel isOpen={isQueueOpen} onClose={() => setIsQueueOpen(false)} triggerRef={queueTriggerRef} />
                            </div>
                        </div>
                        <button onClick={async () => {
                            try { await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/reset`); } catch(err) {}
                ...
                <main className={`flex-1 bg-white border-l border-[#f0f0f0] overflow-hidden ${isSidebarOpen ? 'rounded-tl-[24px]' : ''}`}>
                    <div className="h-full overflow-y-auto bg-white"><Outlet /></div>
                </main>
            </div>
